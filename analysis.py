...
def plot_fastest_sectors(session, driver1, driver2, lap_selection='fastest'):
    ...
    average_speed = telemetry.groupby(['Driver', 'Minisector'])['Speed'].mean().reset_index()
    ...
    for minisector in range(1, total_minisectors + 1):
        speed_d1 = average_speed[(average_speed['Driver'] == driver1) & 
                                (average_speed['Minisector'] == minisector)]['Speed'].values[0]
        speed_d2 = average_speed[(average_speed['Driver'] == driver2) & 
                                (average_speed['Minisector'] == minisector)]['Speed'].values[0]
        
        fastest = driver1 if speed_d1 > speed_d2 else driver2
        telemetry.loc[telemetry['Minisector'] == minisector, 'Fastest Driver'] = fastest
        telemetry.loc[telemetry['Minisector'] == minisector, 'Fastest Driver int'] = 1 if fastest == driver1 else 2
    ...
    # Add title
    lap_text = "Fastest Laps" if lap_selection == 'fastest' else f"Laps: {driver1}:{lap_selection[0]}, {driver2}:{lap_selection[1]}"
    plt.suptitle(f"Fastest Sectors Comparison - {lap_text}\n{session.event.year} {session.event['EventName']}")
...
def plot_sectors(session, driver1, driver2):
    ...
    plt.suptitle(f"Sector Times Comparison\n{session.event.year} {session.event['EventName']}")
...
def plot_speed_map(session, driver):
    ...
    legend = matplotlib.colorbar.ColorbarBase(cbaxes, norm=normlegend, cmap='plasma',
...
def main():
    ...
    with col1:
        ...
        settings_tab, driver_tab, analysis_tab = st.tabs(["Session", "Drivers", "Analysis"])
        
        with settings_tab:
            year = st.selectbox('Year', range(2018, 2025), label_visibility="collapsed")
            events = get_available_events(year)
            gp = st.selectbox('Grand Prix', events, label_visibility="collapsed")
            session_type = st.selectbox('Session', ['FP1', 'FP2', 'FP3', 'Sprint', 'Q', 'R'], 
        ...
        try:
            ...
            with driver_tab:
                ...
                driver1_name = st.selectbox('Driver 1', list(drivers_info.keys()), 
                                          label_visibility="collapsed")
                remaining_drivers = {k: v for k, v in drivers_info.items() if k != driver1_name}
                driver2_name = st.selectbox('Driver 2', list(remaining_drivers.keys()), 
                                          label_visibility="collapsed")
            ...
            with analysis_tab:
                ...
                analysis_type = st.selectbox('Analysis Type', list(analysis_types.keys()), 
                                           label_visibility="collapsed")
                ...
                if analysis_type == 'Fastest Sectors':
                    ...
                    if not use_fastest:
                        laps_d1 = session.laps.pick_driver(driver1)['LapNumber'].tolist()
                        lap_d1 = st.selectbox(f'Lap ({driver1})', laps_d1, 
                                            label_visibility="collapsed")
                        laps_d2 = session.laps.pick_driver(driver2)['LapNumber'].tolist()
                        lap_d2 = st.selectbox(f'Lap ({driver2})', laps_d2, 
                                            label_visibility="collapsed")
                
                # Add driver selection for Speed Map
                if analysis_type == 'Speed Map':
                    selected_driver = st.radio('Driver for Speed Map', 
            ...
            with st.spinner('Generating plot...'):
                try:
                    if analysis_type == 'Fastest Sectors':
                        ...
                        fig = plot_fastest_sectors(session, driver1, driver2, lap_selection)
